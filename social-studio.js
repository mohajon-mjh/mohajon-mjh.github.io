...
var items=[],catSlug="",secCleared=false,blobUrl=null,blobObj=null,CLOUD=null,PRESET=null,musicBuf=null;
...
function detCat(v){
 var s=norm(v);
 if(s.indexOf("trend")>-1||s.indexOf("trand")>-1)return{type:"flag",key:"isTrending"};
 if(s.indexOf("feature")>-1)return{type:"flag",key:"isFeatured"};
 if(s.indexOf("flash")>-1)return{type:"flag",key:"isFlashSale"};
 if(s.indexOf("deal")>-1)return{type:"flag",key:"isDealsOfDay"};
 if(s.indexOf("coming")>-1)return{type:"flag",key:"isComingSoon"};
 return{type:"gc",slug:v.toLowerCase().trim().replace(/\s+/g,"_")};
}
...
function openStudio(){
 ...
 +'<h2 style="margin:0 0 10px">📱 Bulk Product + Social Studio v7</h2>'
 ...
 +'<div style="display:flex;gap:6px;margin-top:6px"><input id="ssCat" placeholder="সেকশন: Trending Products / Featured / Flash Sale / Deals / Coming Soon / বা ক্যাটাগরি নাম" style="flex:1;padding:8px"><button id="ssCatSave" style="background:#16a34a;color:#fff;border:none;padding:9px 12px;border-radius:8px;font-weight:700">ক্যাটাগরি সেভ</button></div>'
 ...
 document.getElementById("ssCatSave").onclick=function(){
  ...
  if(!v){toast("❌ সেকশন/ক্যাটাগরি লিখুন");return;}
  catSlug=v;secCleared=false;
  var det=detCat(v);
  toast("✅ "+(det.type==="flag"?"সেকশন: "+det.key:"ক্যাটাগরি: "+det.slug));
 };
...
function savePrices(){
 ...
 if(matched===0&&pairs.length){items.forEach(function(it,i){if(pairs[i])it.price=pairs[i].p;});}
 renderList();
 toast("✅ দাম: মিল="+matched+" জোড়া="+pairs.length+(matched===0&&pairs.length?" (ক্রমে বসল)":""));
}
function onFiles(){
...
function saveItem(i){
 ...
 return Promise.all([fb(),imgP]).then(function(R){
  ...
  var det=detCat(catSlug||"");
  var prod=prodObj(it,imgUrl);
  if(det.type==="flag")prod[det.key]=true;else prod.categoryId=det.slug;
  var ref=F.D.push(F.D.ref(F.db,"products"));
  return F.D.set(ref,prod).then(function(){
   it.saved=true;it.pid=ref.key;
   var extra=det.type==="flag"?" ["+det.key+"]":" → "+det.slug;
   if(det.type==="gc"){return F.D.set(F.D.ref(F.db,"settings/globalCategoryProducts/"+det.slug+"/"+ref.key),true).then(function(){toast("✅ সেভ: "+it.name+extra);});}
   toast("✅ সেভ: "+it.name+extra);
  });
 }).catch(function(e){toast("❌ সেভ ব্যর্থ: "+it.name+" ("+(e&&e.message?e.message:e)+")");});
}
function clearOldFlag(key){
 return fb().then(function(F){
  return F.D.get(F.D.ref(F.db,"products")).then(function(snap){
   var s=snap.val()||{},tasks=[];
   Object.keys(s).forEach(function(id){
    if(s[id]&&s[id][key]===true)tasks.push(F.D.update(F.D.ref(F.db,"products/"+id),(function(){var o={};o[key]=null;return o;})()));
   });
 ...
 if(!secCleared){
  ...
  if(det.type==="flag"){
   chain=chain.then(function(){
    document.getElementById("ssProg").textContent="⏳ পুরনো "+det.key+" মুছা হচ্ছে...";
    return clearOldFlag(det.key).then(function(n){document.getElementById("ssProg").textContent="✅ "+n+" টি পুরনো সরানো হয়েছে";});
   });
  }else{
   chain=chain.then(function(){return fb().then(function(F){return F.D.set(F.D.ref(F.db,"settings/globalCategoryProducts/"+det.slug),null);});});
  }
 }
 mk.forEach(function(it){chain=chain.then(function(){return saveItem(items.indexOf(it));});});
 chain.then(function(){renderList();toast("✅ সব পণ্য সেভ! হোমপেজে দেখুন");}).catch(function(e){toast("❌ ব্যর্থ: "+(e&&e.message?e.message:e));});
}
function fixHeavy(){
...
function makeVideo(){
 ...
 var startRec=function(){
  ...
  rec.onstop=function(){
   ...
   document.getElementById("ssProg").textContent="✅ ভিডিও রেডি! 📤 শেয়ার / 📢 পোস্ট";
