...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "gardening_outdoor_living") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} gardening_outdoor_living products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Gardening & Outdoor Living products written successfully.");
  } catch (e) {
