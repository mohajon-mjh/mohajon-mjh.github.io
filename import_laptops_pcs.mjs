...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "laptops_pcs") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} laptops_pcs products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Laptops & PCs products written successfully.");
  } catch (e) {
