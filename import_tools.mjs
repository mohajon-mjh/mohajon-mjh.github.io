...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "power_tools_hand_tools") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} power_tools_hand_tools products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Power Tools & Hand Tools products written successfully.");
  } catch (e) {
