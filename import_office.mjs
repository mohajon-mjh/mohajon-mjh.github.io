...
for (const filePath of files) {
  ...
  for (const [sku, prod] of Object.entries(fileData)) {
    if (prod.categoryId === "office_school_supplies") {
      updates[sku] = prod;
      count++;
    }
  }
...
async function run() {
  try {
    console.log(`Pushing ${count} Office & School Supplies products to Firebase...`);
    await db.ref("products").update(updates);
    console.log("✅ Office & School Supplies updated successfully in Firebase.");
  } catch (e) {
