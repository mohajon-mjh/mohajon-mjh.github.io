...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "sports_outdoors_fitness") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} sports_outdoors_fitness products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Sports & Outdoors, Fitness products written successfully.");
  } catch (e) {
