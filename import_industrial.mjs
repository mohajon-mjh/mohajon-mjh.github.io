...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "industrial_machinery_equipment") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} industrial_machinery_equipment products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Industrial Machinery & Equipment products written successfully.");
  } catch (e) {
