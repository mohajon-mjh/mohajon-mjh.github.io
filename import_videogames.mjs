...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "video_games_consoles") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} video_games_consoles products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Video Games & Consoles products written successfully.");
  } catch (e) {
