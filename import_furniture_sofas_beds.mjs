...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "furniture_sofas_beds_etc") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} furniture_sofas_beds_etc products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Furniture (Sofas, Beds, etc.) products written successfully.");
  } catch (e) {
