...
try:
    ...
    # Firebase DB dump - multiple methods try
    print("⏳ Firebase DB dump...")
    
    # Method 1: export format
    try:
        db_url = f"{DB_URL}/.json?format=export&auth={token}"
        req = urllib.request.Request(db_url)
        firebase_db = urllib.request.urlopen(req, timeout=180).read()
        print(f"✅ DB (export): {len(firebase_db)//1024} KB")
    except Exception as e:
        print(f"⚠️ Method 1 failed: {e}")
        # Method 2: regular json
        try:
            db_url = f"{DB_URL}/.json?auth={token}"
            req = urllib.request.Request(db_url)
            firebase_db = urllib.request.urlopen(req, timeout=180).read()
            print(f"✅ DB (regular): {len(firebase_db)//1024} KB")
    ...
    try:
        rules_url = f"{DB_URL}/.settings/rules.json?auth={token}"
        req = urllib.request.Request(rules_url)
        firebase_rules = urllib.request.urlopen(req, timeout=60).read()
        print("✅ Rules saved")
    except Exception as e:
        print(f"⚠️ Rules failed: {e}")
...
with tarfile.open(TAR_FILE, "w:gz") as tar:
    ...
    if firebase_db:
        ...
        print("⚠️ Firebase DB backup skipped")
    ...
    manifest = {
        ...
        "has_firebase_db": firebase_db is not None,
        "has_firebase_rules": firebase_rules is not None,
        "contains": ["site files", "full git repo"] + (["firebase database"] if firebase_db else []) + (["firebase rules"] if firebase_rules else [])
    }
...
print(f"📊 Firebase DB: {'✅ আছে' if firebase_db else '❌ নেই'}")
print(f"📊 Firebase Rules: {'✅ আছে' if firebase_rules else '❌ নেই'}")
