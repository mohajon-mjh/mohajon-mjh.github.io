...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "toys_games_hobbies") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} toys_games_hobbies products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Toys, Games & Hobbies products written successfully.");
  } catch (e) {
