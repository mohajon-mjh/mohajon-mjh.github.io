...
for (const filePath of files) {
  ...
  for (const [sku, prod] of Object.entries(fileData)) {
    if (prod.categoryId === "safety_security") {
      updates[sku] = prod;
      count++;
    }
  }
...
async function run() {
  try {
    console.log(`Pushing ${count} Safety & Security products to Firebase...`);
    await db.ref("products").update(updates);
    console.log("✅ Safety & Security updated successfully in Firebase.");
  } catch (e) {
