...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "smart_home_surveillance") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} smart_home_surveillance products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Smart Home & Surveillance products written successfully.");
  } catch (e) {
