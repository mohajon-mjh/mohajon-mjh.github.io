...
function row(grid,ids){var n=0;var tmp=document.createDocumentFragment();ids.forEach(function(id){var p=P[id];if(!p)return;tmp.appendChild(card(id,p));n++;});if(n){grid.innerHTML="";grid.appendChild(tmp);}else if(!grid.children.length){grid.innerHTML='<div style="padding:16px;color:#888">পণ্য নেই</div>';}}
