...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "home_kitchen") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} home_kitchen products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Home & Kitchen products written successfully.");
  } catch (e) {
