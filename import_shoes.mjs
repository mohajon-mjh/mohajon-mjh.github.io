...
for (const filePath of files) {
  ...
  for (const [sku, prod] of Object.entries(fileData)) {
    if (prod.categoryId === "shoes_accessories") {
      updates[sku] = prod;
      count++;
    }
  }
...
async function run() {
  try {
    console.log(`Pushing ${count} Shoes & Accessories products to Firebase...`);
    await db.ref("products").update(updates);
    console.log("✅ Shoes & Accessories updated successfully in Firebase.");
  } catch (e) {
