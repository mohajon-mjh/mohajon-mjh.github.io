...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "health_medical_supplies") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} health_medical_supplies products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Health & Medical Supplies products written successfully.");
  } catch (e) {
