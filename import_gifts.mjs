...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "gifts_crafts") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} gifts_crafts products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Gifts & Crafts products written successfully.");
  } catch (e) {
