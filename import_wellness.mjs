...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "health_wellness") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} health_wellness products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Health & Wellness products written successfully.");
  } catch (e) {
