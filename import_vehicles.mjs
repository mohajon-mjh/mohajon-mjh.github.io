...
for (const [sku, prod] of Object.entries(file1)) {
  if (prod.categoryId === "vehicles_transportation") toUpdate[sku] = prod;
}
...
async function run() {
  try {
    console.log(`Writing ${Object.keys(toUpdate).length} vehicles_transportation products to Firebase...`);
    await db.ref("products").update(toUpdate);
    console.log("✅ Vehicles & Transportation products written successfully.");
  } catch (e) {
