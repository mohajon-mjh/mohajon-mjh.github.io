/* MJH Home Unified Loader v1 — এক সিস্টেম, এক নিয়ম */
(function(){
"use strict";
var CK="mjh_home_cache",CT="mjh_home_cache_t",ALL=[],lastCat=null;
function slim(p){return {id:p.id,title:p.title,price:p.price,discountPrice:p.discountPrice,oldPrice:p.oldPrice,stock:p.stock,status:p.status,category:p.category,categoryId:p.categoryId,images:p.images,image:p.image,isTrending:p.isTrending,isFeatured:p.isFeatured,isFlashSale:p.isFlashSale,isRecommended:p.isRecommended};}
function load(cb){
...
function card(p){
 ...
 return '<a href="product-details.html?id='+p.id+'" style="text-decoration:none;color:inherit"><div style="min-width:170px;max-width:170px;background:#fff;border-radius:12px;box-shadow:0 2px 10px rgba(0,0,0,.08);overflow:hidden;margin-right:12px;flex:0 0 auto">'+
 '<div style="position:relative"><img src="'+img+'" loading="lazy" style="width:100%;height:130px;object-fit:contain;background:#f1f3f5;display:block">'+
 (p.stock>0?'<span style="position:absolute;top:6px;left:6px;background:#22c55e;color:#fff;font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px">IN STOCK</span>':'<span style="position:absolute;top:6px;left:6px;background:#ef4444;color:#fff;font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px">OUT OF STOCK</span>')+
...
function fill(el,list){
 if(!el)return;
 el.innerHTML="";
 if(!list.length){el.style.display="block";el.innerHTML='<p style="text-align:center;color:#888;padding:20px">📦 এই section-এ এখনো পণ্য যোগ করা হয়নি — Admin → All Category থেকে পণ্য যোগ করুন</p>';return;}
 el.style.display="flex";el.style.overflowX="auto";el.style.padding="8px 0";
 list.slice(0,30).forEach(function(p){el.insertAdjacentHTML("beforeend",card(p));});
}
function byFlag(f){return ALL.filter(function(p){return active(p)&&p[f]===true;});}
function byCat(cat){return ALL.filter(function(p){return active(p)&&((p.categoryId||"")===cat||(p.category||"")===cat||String(p.category||"").toLowerCase()===String(cat).toLowerCase());});}
...
function run(){
 ...
 setTimeout(function(){
  ["trendingProductsGrid","flashSaleProductsGrid","dealsGrid","globalCatCarousel","featuredProducts","specialCatCarousel"].forEach(function(id){
   ...
   if(el&&/লোড হচ্ছে/.test(el.textContent)){el.style.display="block";el.innerHTML='<p style="text-align:center;color:#888;padding:20px">📦 এই section-এ এখনো পণ্য যোগ করা হয়নি</p>';}
  });
 },6000);
}
